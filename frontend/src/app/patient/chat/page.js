...
export default function PatientChatPage() {
  ...
  const openChat = async (doctorIdParam) => {
    try {
      const { data } = await api.get(`/chats/${doctorIdParam}`);
  ...
  const refreshCurrentChat = async () => {
    ...
    try {
      const { data } = await api.get(`/chats/${selectedChat.doctor._id}`);
      setSelectedChat(data.data);
    } catch (error) {
              ...
              <div className="card p-0 overflow-hidden flex flex-col h-[600px]">
                ...
                <div className="p-4 bg-primary text-white flex items-center gap-3">
                  <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center overflow-hidden">
                    {selectedChat.doctor?.avatar ? (
                      <img
                        src={selectedChat.doctor.avatar}
                        alt={selectedChat.doctor.name}
                        className="w-full h-full object-cover"
                      />
                    ) : (
                  ...
                  <div>
                    <h3 className="font-semibold">Dr. {selectedChat.doctor?.name}</h3>
                    <p className="text-xs opacity-90">{selectedChat.doctor?.specialization}</p>
                  </div>
                </div>
